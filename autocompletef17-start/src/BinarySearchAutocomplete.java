...
public class BinarySearchAutocomplete implements Autocompletor {
	...
	public BinarySearchAutocomplete(String[] terms, double[] weights) {
		...
		for (int i = 0; i < terms.length; i++) {
			if(weights[i]<0) {
				throw new IllegalArgumentException("negative weight in the array");
			}
	...
	public static int firstIndexOf(Term[] a, Term key, Comparator<Term> comparator) {
		...
		while((highInd-lowInd)<=1) {
			if(comparator.compare(key, a[index])>0) {
				lowInd = index;
				index = (highInd-lowInd)/2;
			}
			else {
				highInd = index;
				index = (highInd-lowInd)/2;
	...
	//CONTINUE FROM HERE, TOP MATCH WEIGHT OF FIRST MAYBE
	public Iterable<String> topMatches(String prefix, int k) {
		ArrayList<Term> result = new ArrayList<Term>();
		int start = firstIndexOf(myTerms,prefix,compare);
		int end = lastIndexOf(myTerms,prefix,compare);
		for(int n=start; n<end; n++) {
			result.add(myTerms[n]);
		}
		Collections.sort()
		return null;
	}
	...
	public double weightOf(String term) {
		int len = myTerms.length;
		int index = len/2;
		for(myTerms[index].getWord()!=term) {
			//NONONONNO USE COMPARATOR IF YOU CAN
			if(myTerms[index].getWord().compareTo(term)>0) {
