...
public class Term implements Comparable<Term> {
	...
	public Term(String word, double weight) {
		...
		if(word==null) throw   new   NullPointerException("null word");
		myWord = word;
		myWeight = weight;
	}
	...
	public static class PrefixOrder implements Comparator<Term> {
		...
		public int compare(Term v, Term w) {
			...
			int endIndex = r;
			int len = Math.min(v.getWord().length(), w.getWord().length());
			if(len<r) {endIndex = len;}
			String vTemp = v.getWord().substring(0,endIndex);
			String wTemp = w.getWord().substring(0,endIndex);
			if(vTemp.compareTo(wTemp)!=0 || endIndex==r) {return vTemp.compareTo(wTemp);}
			else{return v.getWord().length()-w.getWord().length();}
		}
	}
